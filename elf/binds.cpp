...
ELF_API elf_handle ELF_APIENTRY elfGetActorParent(elf_handle actor)
{
	...
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: GetActorParent() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API void ELF_APIENTRY elfSetActorParent(elf_handle actor, elf_handle parent)
{
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: SetActorParent() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API int ELF_APIENTRY elfGetActorChildCount(elf_handle actor)
{
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: GetActorChildCount() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API elf_handle ELF_APIENTRY elfGetActorChildByName(elf_handle actor, const char* name)
{
	...
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: GetActorChildByName() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API elf_handle ELF_APIENTRY elfGetActorChildByIndex(elf_handle actor, int idx)
{
	...
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: GetActorChildByIndex() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API void ELF_APIENTRY elfRemoveActorChildren(elf_handle actor)
{
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: RemoveActorChildren() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
...
ELF_API void ELF_APIENTRY elfSetActorHierarchyIpoFrame(elf_handle actor, float frame)
{
	if(!actor.get() || !elf_is_actor(actor.get()))
	{
		...
		{
			int line = elf_get_current_script_line();
			elf_set_error_no_save(ELF_INVALID_HANDLE, "[script \"%s\" %s]:%d: SetActorHierarchyIpoFrame() -> invalid handle\n", elf_get_script_name(script), elf_get_script_file_path(script), line);
		}
		else
		{
