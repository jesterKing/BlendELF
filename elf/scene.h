...
unsigned char elf_script_in_list(elf_list *scripts, elf_script *script)
{
	elf_script *scr;

	for(scr = (elf_script*)elf_begin_list(scripts); scr;
		scr = (elf_script*)elf_next_in_list(scripts))
	{
		if(scr->id == script->id)
		{
			return ELF_TRUE;
...
unsigned char elf_texture_in_list(elf_list *textures, elf_texture *texture)
{
	elf_texture *tex;

	for(tex = (elf_texture*)elf_begin_list(textures); tex;
		tex = (elf_texture*)elf_next_in_list(textures))
	{
...
unsigned char elf_material_in_list(elf_list *materials, elf_material *material)
{
	elf_material *mat;

	for(mat = (elf_material*)elf_begin_list(materials); mat;
		mat = (elf_material*)elf_next_in_list(materials))
	{
...
unsigned char elf_model_in_list(elf_list *models, elf_model *model)
{
	elf_model *mdl;

	for(mdl = (elf_model*)elf_begin_list(models); mdl;
		mdl = (elf_model*)elf_next_in_list(models))
	{
		if(mdl->id == model->id)
		{
			return ELF_TRUE;
...
unsigned char elf_armature_in_list(elf_list *armatures, elf_armature *armature)
{
	elf_armature *arm;

	for(arm = (elf_armature*)elf_begin_list(armatures); arm;
		arm = (elf_armature*)elf_next_in_list(armatures))
	{
		if(arm->id == armature->id)
		{
			return ELF_TRUE;
		}
	}
...
unsigned char elf_save_scene(elf_scene *scene, const char *file_path)
{
	...
	{
		if(cam->script && !elf_script_in_list(scripts, cam->script))
		{
		...
		ent = (elf_entity*)elf_next_in_list(scene->entities))
	{
		...
		{
			if(!elf_material_in_list(materials, mat))
			{
				...
				{
					if(mat->textures[i] && !elf_texture_in_list(textures, mat->textures[i]))
					{
	...
	{
		if(lig->script && !elf_script_in_list(scripts, lig->script))
	...
	{
		if(par->script && !elf_script_in_list(scripts, par->script))
	...
	{
		if(spr->script && !elf_script_in_list(scripts, spr->script))
